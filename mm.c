...
#define NEXT_PTR(bp) ((char *)GET(NEXTP(bp)))
#define PREV_PTR(bp) ((char *)GET(PREVP(bp)))
...
void list_add_(void ** ptr_head, void *p) {
    if (*ptr_head) {
        PUT(PREVP(*ptr_head), (int)p);
    }

    PUT(NEXTP(p), (int)(*ptr_head));
    PUT(PREVP(p), 0);
    *ptr_head = p;
}
...
void list_del_(void ** ptr_head, void *p) {
    if (PREV_PTR(p)) {
        PUT(NEXTP(PREV_PTR(p)), (int)NEXT_PTR(p));
    }
    if (NEXT_PTR(p)) {
        PUT(PREVP(NEXT_PTR(p)), (int)PREV_PTR(p));
    }
    if (*ptr_head == p) {
        *ptr_head = NEXT_PTR(p);
    }
